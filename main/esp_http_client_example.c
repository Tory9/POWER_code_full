...
char* print_time() {
    static char strftime_buf[64];  // Static so it persists after function ends
    ...
    // Normalize in case of overflow (e.g. from 23:00 to 00:00 next day)
    mktime(&timeinfo);
...
static void https_with_url(void)
{
    esp_http_client_config_t config = {
        .url = "https://ee3-energy1-default-rtdb.europe-west1.firebasedatabase.app/data.json?auth=jQpI4wE58kv5GlcDeiswL7vaH32CD0OW48iJemVH",
...
void send_sensor_data(int ina_out, float power, float voltage){
    ...
    const char* base_path = (ina_out == 1) ? "powerIn" : "powerOut";
    snprintf(url, sizeof(url), 
             "https://ee3-energy1-default-rtdb.europe-west1.firebasedatabase.app/data/%s/%s.json?auth=jQpI4wE58kv5GlcDeiswL7vaH32CD0OW48iJemVH", 
             base_path, time_str);
